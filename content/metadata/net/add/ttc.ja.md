...
head_title: "C＃.NETアプリケーションのTTCファイルにメタデータを追加する"
head_description: "C＃.NETメタデータ処理APIを使用して、メタデータ情報をTTCファイルに追加します。メタデータ標準XMP、EXIF、IPTC、ID3などを操作します。"

title: "C＃でTTCにメタデータを追加する"
...
steps:
    enable: true
    title_left: "C＃でTTCメタデータを追加する手順"
    content_left: |
        [GroupDocs.Metadata](/metadata/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーション内からTTCファイルにメタデータの詳細を簡単に追加できます。
...
demos:
    ...
    content: |
        [GroupDocs.Metadata Live Demos](https://products.groupdocs.app/metadata/family)Webサイトにアクセスして、今すぐTTCファイルにメタデータ情報を追加してください。  
        ライブデモには次の利点があります
