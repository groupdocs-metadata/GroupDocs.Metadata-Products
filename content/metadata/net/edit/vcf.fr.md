...
############################# Head ############################
head_title: "Modifier les métadonnées dans les fichiers Vcf dans les applications C#"
head_description: "C# API de traitement des métadonnées pour modifier les informations de métadonnées dans les fichiers Vcf. Travaillez avec les normes de métadonnées XMP, EXIF, IPTC, ID3, etc."

############################# Header ############################
title: "Mettre à jour les métadonnées du fichier Vcf dans C#" 
...
steps:
    enable: true
    title: "Étapes de mise à jour des métadonnées vers VCF dans C#"
    content: |
      [GroupDocs.Metadata for .NET](https://products.groupdocs.com/metadata/net/) permet aux développeurs .NET de modifier facilement les informations de métadonnées dans les fichiers VCF à partir de leurs applications en mettant en œuvre quelques étapes simples.
      
      1. Chargez le fichier VCF via une instance de la classe Metadata.
      ...
      4. Enregistrez les modifications sur le disque au format VCF.
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        // Modifier la date de création de VCF
...
more_features:
  enable: true
  title: "Gérer les VCF propriétés de métadonnées internes pour les applications .NET"
  ...
  features:
    ...
    # feature loop
    - title: "Tirer parti des propriétés VCF intégrées"
