...
############################# Head ############################
head_title: "C# アプリケーションの XLSX ファイルへのメタデータの編集"
head_description: "C# メタデータ処理APIを使用してメタデータ情報を XLSX ファイルに編集します。メタデータ規格 XMP、EXIF、IPTC、ID3 などと連携してください。"

############################# Header ############################
title: "C# 内の XLSX ファイルのメタデータを更新"
...
steps:
    enable: true
    title_left: "C# のメタデータを XLSX に更新する手順"
    content_left: |
        [GroupDocs.Metadata for .NET](/ja/metadata/net/) を使用すると、.NET 開発者はいくつかの簡単な手順を実行することで、アプリケーション内からメタデータ情報を XLSX ファイルに簡単に編集できます。
        
        * Metadata クラスのインスタンス経由で XLSX ファイルをロードします。
        * 必要なメタデータプロパティをフィルタリングするために使用する述語を指定します。
        * 述語と新しい値を updateProperties メソッドに渡します。
        * 変更を XLSX 形式でディスクに保存します。
...
demos:
    ...
    content: |
       [GroupDocs.Metadata ライブデモ](https://products.groupdocs.app/metadata/family) ウェブサイトにアクセスして、メタデータ情報を XLSX ファイルに今すぐ更新してください。
       ライブデモには次の利点があります。
