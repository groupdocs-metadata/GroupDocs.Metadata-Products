...
############################# Head ############################
head_title: "在 C# 应用程序中将元数据编辑为 Torrent 文件"
head_description: "C# 元数据处理 API，用于将元数据信息编辑到 Torrent 文件。使用元数据标准 XMP、EXIF、IPTC、ID3 等。"

############################# Header ############################
title: "更新 C# 中 Torrent 文件的元数据" 
...
steps:
    enable: true
    title: "将元数据更新为 C# 中的 TORRENT 的步骤"
    content: |
      [GroupDocs.Metadata for .NET](https://products.groupdocs.com/metadata/net/) 使 .NET 开发人员可以通过执行几个简单的步骤，轻松地将元数据信息从其应用程序内编辑到 TORRENT 文件。
      
      1. 通过 Metadata 类的实例加载 TORRENT 文件。
      2. 指定将用于过滤所需元数据属性的谓词。
      3. 将谓词和新值传递给 UpdateProperties 方法。
      4. 以 TORRENT 格式将更改保存到光盘。
...
more_features:
  enable: true
  title: "管理 .NET 应用的 TORRENT 内部元数据属性"
