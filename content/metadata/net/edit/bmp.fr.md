...
############################# Head ############################
head_title: "Modifier les métadonnées dans les fichiers Bmp dans les applications C#"
head_description: "C# API de traitement des métadonnées pour modifier les informations de métadonnées dans les fichiers Bmp. Travaillez avec les normes de métadonnées XMP, EXIF, IPTC, ID3, etc."

############################# Header ############################
title: "Mettre à jour les métadonnées du fichier Bmp dans C#" 
...
steps:
    enable: true
    title: "Étapes de mise à jour des métadonnées vers BMP dans C#"
    content: |
      [GroupDocs.Metadata for .NET](https://products.groupdocs.com/metadata/net/) permet aux développeurs .NET de modifier facilement les informations de métadonnées dans les fichiers BMP à partir de leurs applications en mettant en œuvre quelques étapes simples.
      
      1. Chargez le fichier BMP via une instance de la classe Metadata.
      ...
      4. Enregistrez les modifications sur le disque au format BMP.
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        // Modifier la date de création de BMP
...
more_features:
  enable: true
  title: "Gérer les BMP propriétés de métadonnées internes pour les applications .NET"
  ...
  features:
    ...
    # feature loop
    - title: "Tirer parti des propriétés BMP intégrées"
