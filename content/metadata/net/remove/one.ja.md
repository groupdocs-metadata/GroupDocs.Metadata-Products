...
head_title: "C＃.NETアプリケーションで1つのファイルのメタデータを削除する"
head_description: "ONEファイルのメタデータ情報を削除するC＃.NETメタデータ処理API。メタデータ標準XMP、EXIF、IPTC、ID3などを操作する."

title: "C＃で1つのファイルからメタデータを削除する"
...
steps:
    enable: true
    title_left: "C＃で1つのメタデータを削除する手順"
    content_left: |
        [GroupDocs.Metadata](/metadata/net/)を使用すると、.NET開発者は、いくつかの簡単な手順を実装することで、アプリケーション内から1つのファイルからメタデータの詳細を簡単に削除できます。
