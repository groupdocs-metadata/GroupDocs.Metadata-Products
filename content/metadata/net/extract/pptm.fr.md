...
############################# Head ############################
head_title: "Lire et extraire les métadonnées des fichiers PPTM dans les applications C#"
head_description: "API de gestion des métadonnées multiplateforme C# pour lire et extraire les informations de métadonnées des fichiers PPTM. Travaillez avec les normes de métadonnées XMP, EXIF, IPTC, ID3, etc."

############################# Header ############################
title: "Extraire les métadonnées du fichier PPTM dans C#" 
...
steps:
    enable: true
    title: "Étapes pour l'extraction de métadonnées PPTM dans .NET"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/net/) permet aux développeurs .NET d'extraire, de lire et d'extraire facilement les informations de métadonnées des fichiers PPTM à partir de leurs applications en mettant en œuvre quelques étapes simples.
      
      1. Chargez le PPTM avec une instance de la classe .NET.
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        // Extraire les propriétés de métadonnées PPTM selon divers critères

        // Construire Metadata en passant le chemin PPTM
