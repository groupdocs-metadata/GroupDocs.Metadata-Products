...
############################# Head ############################
head_title: "在 C# 应用程序中读取并提取 TORRENT 文件的元数据"
head_description: "跨平台C#元数据管理API，用于读取和提取TORRENT文件的元数据信息。使用元数据标准 XMP、EXIF、IPTC、ID3 等。"

############################# Header ############################
title: "从 C# 中的 TORRENT 文件提取元数据" 
...
steps:
    enable: true
    title: "在 .NET 中提取 TORRENT 元数据的步骤"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/net/) 使 .NET 开发人员可以通过实施几个简单的步骤，轻松地从应用程序内的 TORRENT 文件中读取和提取元数据信息。
      
      1. 使用 .NET 类的实例加载 TORRENT。
      2. 组成一个谓词来检查所有元数据属性。
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        // 按各种标准提取 TORRENT 元数据属性

        // 构造 Metadata 传递 TORRENT 路径
