...
head_description: "MPT ファイルのメタデータ情報を読み取り、抽出するクロスプラットフォームの C# メタデータ管理 API。メタデータ標準 XMP、EXIF、IPTC、ID3 などを使用します。"

############################# Header ############################
title: "C# の MPT ファイルからメタデータを抽出する" 
...
steps:
    enable: true
    title: ".NET での MPT メタデータ抽出の手順"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/net/) を使用すると、.NET の開発者は、いくつかの簡単な手順を実装することで、アプリケーション内から MPT ファイルからメタデータ情報を簡単に読み取り、抽出できます。
      
      1. .NET クラスのインスタンスを使用して MPT をロードします。
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        // さまざまな基準に基づいて MPT メタデータ プロパティを抽出します

        // MPT パスを渡す Metadata を構築します
