...
head_description: "XLSX ファイルのメタデータ情報を読み取り、抽出するクロスプラットフォームの C# メタデータ管理 API。メタデータ標準 XMP、EXIF、IPTC、ID3 などを使用します。"

############################# Header ############################
title: "C# の XLSX ファイルからメタデータを抽出する" 
...
steps:
    enable: true
    title: ".NET での XLSX メタデータ抽出の手順"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/net/) を使用すると、.NET の開発者は、いくつかの簡単な手順を実装することで、アプリケーション内から XLSX ファイルからメタデータ情報を簡単に読み取り、抽出できます。
      
      1. .NET クラスのインスタンスを使用して XLSX をロードします。
    ...
    code:
      ...
      content: |
        ```csharp {style=abap}
        // さまざまな基準に基づいて XLSX メタデータ プロパティを抽出します

        // XLSX パスを渡す Metadata を構築します
