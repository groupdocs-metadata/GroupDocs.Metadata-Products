...
head_title: "Javaで1つのファイルのメタデータプロパティを削除する"
head_description: "クロスプラットフォームのJavaメタデータAPIを使用して、1つのファイルのメタデータフィールドを非表示にしたり削除したりします。メタデータ標準XMP、EXIF、IPTC、ID3などを操作します。"

title: "Javaで1つのメタデータを削除する"
description: "GroupDocs.Metadata for Java APIを使用して、ONEおよび他の多くの一般的なドキュメント、画像、マルチメディアファイル形式からメタデータプロパティを削除します."
...
steps:
    enable: true
    title_left: "Javaで1つのメタデータを削除する手順"
    content_left: |
...
about_formats:
    ...
    format:
        - icon: "far fa-file-o"
          title: "1つのファイル形式について"
          content: |
            OneNoteファイル形式は、デジタルノートをセクションとページの階層セットとして表すための効果的な方法を提供します。各ページには、ファイル形式のDocument Object Model（DOM）で表現するための特定の構造のユーザー定義コンテンツが含まれています。この形式のデータモデルは次のとおりです。
