...
############################# Head ############################
head_title: "Supprimer les propriétés de métadonnées des fichiers VCR dans Java"
head_description: "API de métadonnées multiplateforme Java pour masquer et supprimer les champs de métadonnées des fichiers VCR. Travaillez avec les normes de métadonnées XMP, EXIF, IPTC, ID3, etc."

############################# Header ############################
title: "Supprimer les métadonnées VCR dans Java" 
description: "Supprimez les propriétés de métadonnées de VCR et de nombreux autres formats de documents, d'images et de fichiers multimédias populaires à l'aide de l'API GroupDocs.Metadata for Java."
...
steps:
    enable: true
    title: "Étapes pour supprimer les métadonnées de VCR dans Java"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/java/) permet aux développeurs Java de supprimer facilement les informations de métadonnées des fichiers VCR depuis leurs applications en mettant en œuvre quelques étapes simples.
      
      1. Chargez le fichier VCR à mettre à jour.
    ...
    code:
      ...
      content: |
        ```java {style=abap}
        // Effacer les métadonnées du document VCR
