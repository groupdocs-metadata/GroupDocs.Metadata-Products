...
head_title: "表示＆amp; JavaアプリケーションでPNGファイルのメタデータを抽出する"
head_description: "PNGファイルのメタデータ情報を読み取って抽出するクロスプラットフォームのJavaメタデータ抽出API。メタデータ標準XMP、EXIF、IPTC、ID3などを操作します。"

title: "JavaでPNGメタデータを抽出する"
...
steps:
    enable: true
    title_left: "JavaでのPNGメタデータ抽出の手順"
    content_left: |
        [GroupDocs.Metadata](/metadata/java/)を使用すると、Java開発者は、いくつかの簡単な手順を実装することで、アプリケーション内からPNGファイルからメタデータ情報を簡単に取得できます。

        *メタデータプロパティを検索するPNGファイルをロードします。
