...
############################# Head ############################
head_title: "Чтение и извлечение метаданных файлов DICOM в приложениях Java"
head_description: "Межплатформенный API управления метаданными Java для чтения и извлечения информации метаданных из файлов DICOM. Работа со стандартами метаданных XMP, EXIF, IPTC, ID3 и др."

############################# Header ############################
title: "Извлечь метаданные из файла DICOM в Java" 
...
steps:
    enable: true
    title: "Шаги по извлечению метаданных DICOM в Java"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/java/) позволяет разработчикам Java легко извлекать и читать метаданные из файлов DICOM из своих приложений, выполнив несколько простых шагов.
      
      1. Загрузите DICOM с экземпляром класса Java.
    ...
    code:
      ...
      content: |
        ```java {style=abap}
        // Поиск метаданных изображения в документе DICOM

        // Составьте Metadata, передав DICOM в конструктор.
