...
############################# Head ############################
head_title: "Clean Metadata from VCR Files in Python Apps"
head_description: "Use the Python API to strip metadata from VCR files. Supports formats like XMP, EXIF, IPTC, ID3, and others."

############################# Header ############################
title: "Delete Metadata in VCR Using Python" 
...
steps:
    enable: true
    title: "How to Remove Metadata from VCR in Python"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/python-net/) makes it simple for developers to clean metadata from VCR files by following a few steps.
      
      1. Open the VCR file using the Metadata class.
      2. Remove all metadata in the loaded file.
      3. Handle the result of the cleanup process.
      4. Save the updated file in VCR format.
    ...
    code:
      ...
      content: |
        ```python {style=abap}
        # Erase metadata from a VCR file
