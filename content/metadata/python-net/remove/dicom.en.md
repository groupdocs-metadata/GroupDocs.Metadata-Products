...
############################# Head ############################
head_title: "Clean Metadata from DICOM Files in Python Apps"
head_description: "Use the Python API to strip metadata from DICOM files. Supports formats like XMP, EXIF, IPTC, ID3, and others."

############################# Header ############################
title: "Delete Metadata in DICOM Using Python" 
...
steps:
    enable: true
    title: "How to Remove Metadata from DICOM in Python"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/python-net/) makes it simple for developers to clean metadata from DICOM files by following a few steps.
      
      1. Open the DICOM file using the Metadata class.
      2. Remove all metadata in the loaded file.
      3. Handle the result of the cleanup process.
      4. Save the updated file in DICOM format.
    ...
    code:
      ...
      content: |
        ```python {style=abap}
        # Erase metadata from a DICOM file
