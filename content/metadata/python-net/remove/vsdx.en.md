...
############################# Head ############################
head_title: "Clean Metadata from VSDX Files in Python Apps"
head_description: "Use the Python API to strip metadata from VSDX files. Supports formats like XMP, EXIF, IPTC, ID3, and others."

############################# Header ############################
title: "Delete Metadata in VSDX Using Python" 
...
steps:
    enable: true
    title: "How to Remove Metadata from VSDX in Python"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/python-net/) makes it simple for developers to clean metadata from VSDX files by following a few steps.
      
      1. Open the VSDX file using the Metadata class.
      2. Remove all metadata in the loaded file.
      3. Handle the result of the cleanup process.
      4. Save the updated file in VSDX format.
    ...
    code:
      ...
      content: |
        ```python {style=abap}
        # Erase metadata from a VSDX file
