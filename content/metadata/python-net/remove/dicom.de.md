...
############################# Head ############################
head_title: "Metadaten aus DICOM-Dateien in Python-Apps bereinigen"
head_description: "Verwenden Sie die Python API, um Metadaten aus DICOM-Dateien zu entfernen. Unterstützt Formate wie XMP, EXIF, IPTC, ID3 und andere."

############################# Header ############################
title: "Metadaten in DICOM mit Python löschen" 
...
steps:
    enable: true
    title: "So entfernen Sie Metadaten aus DICOM in Python"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/python-net/) ermöglicht es Entwicklern, Metadaten aus DICOM-Dateien durch die Befolgung einiger Schritte zu bereinigen.
      
      1. Öffnen Sie die DICOM-Datei mit der Metadata-Klasse.
      2. Entfernen Sie alle Metadaten in der geladenen Datei.
      3. Verarbeiten Sie das Ergebnis des Bereinigungsprozesses.
      4. Speichern Sie die aktualisierte Datei im DICOM-Format.
    ...
    code:
      ...
      content: |
        ```python {style=abap}
        # Metadaten aus einer DICOM-Datei löschen
