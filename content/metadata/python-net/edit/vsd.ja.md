...
############################# Head ############################
head_title: "VsdファイルのメタデータをPythonを使用して編集する"
head_description: "Python APIを使用してVsdファイルのメタデータを修正します。XMP、EXIF、IPTC、ID3、その他のメタデータ形式をサポートしています。"

############################# Header ############################
title: "PythonでVsdファイルのメタデータを修正する" 
...
steps:
    enable: true
    title: "Pythonを使用してVSDファイルのメタデータを編集する方法"
    content: |
      [GroupDocs.Metadata for Python via .NET](https://products.groupdocs.com/metadata/python-net/)を使用して、Python開発者は以下の簡単な手順に従ってVSDファイルのメタデータを更新できます：
      
      1. Metadataクラスを使用してVSDファイルを読み込む。
      2. 変更したいメタデータをフィルタリングする条件を定義する。
      3. フィルターを適用し、新しい値を設定する。
      4. 更新されたVSDファイルをディスクに保存する。
...
more_features:
  enable: true
  title: "PythonでVSDファイルのメタデータを編集する"
