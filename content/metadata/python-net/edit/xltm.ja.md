...
############################# Head ############################
head_title: "XltmファイルのメタデータをPythonを使用して編集する"
head_description: "Python APIを使用してXltmファイルのメタデータを修正します。XMP、EXIF、IPTC、ID3、その他のメタデータ形式をサポートしています。"

############################# Header ############################
title: "PythonでXltmファイルのメタデータを修正する" 
...
steps:
    enable: true
    title: "Pythonを使用してXLTMファイルのメタデータを編集する方法"
    content: |
      [GroupDocs.Metadata for Python via .NET](https://products.groupdocs.com/metadata/python-net/)を使用して、Python開発者は以下の簡単な手順に従ってXLTMファイルのメタデータを更新できます：
      
      1. Metadataクラスを使用してXLTMファイルを読み込む。
      2. 変更したいメタデータをフィルタリングする条件を定義する。
      3. フィルターを適用し、新しい値を設定する。
      4. 更新されたXLTMファイルをディスクに保存する。
...
more_features:
  enable: true
  title: "PythonでXLTMファイルのメタデータを編集する"
