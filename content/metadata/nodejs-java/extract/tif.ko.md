...
############################# Head ############################
head_title: "Node.js 앱의 TIF 파일에서 메타데이터 읽기"
head_description: "Node.js 전체의 TIF 파일에서 메타데이터에 쉽게 액세스하고 추출할 수 있습니다. XMP, EXIF, IPTC, ID3와 같은 일반적인 형식으로 작동합니다."

############################# Header ############################
title: "TIF 파일에서 메타데이터 추출" 
...
steps:
    enable: true
    title: "GroupDocs.Metadata을(를) 사용하여 Node.js의 TIF 파일에서 메타데이터를 추출하는 방법"
    content: |
      [GroupDocs.Metadata](https://products.groupdocs.com/metadata/nodejs-java/)를 사용하여 Node.js via Java 애플리케이션 내의 TIF 파일에서 메타데이터를 쉽게 추출할 수 있습니다. 방법은 다음과 같습니다.
      
      1. 메타데이터를 추출하려는 TIF 파일을 로드합니다.
    ...
    code:
      ...
      content: |
        ...
        // TIF 파일에서 모든 세부정보를 추출합니다.

        // 생성자에 TIF을 전달하여 Metadata를 작성합니다.
